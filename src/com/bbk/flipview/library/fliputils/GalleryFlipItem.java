 ...
 * <p/>
 * Only corrected its async loading logic, although its layout hierarchy can be
 * optimized too.
 */
public class GalleryFlipItem extends LinearLayout
{
	...
	// Change to a static inner class from a normal inner class, otherwise it
	// may holds a strong reference to GalleryFlipItem, which is not a good
	// practise for AsyncTask
	private static class ContentPhotosDownloader extends
			AsyncTask<Void, Void, Bitmap>
	{
		...
		public ContentPhotosDownloader(String url, ImageView imgView,
				ProgressBar progressBar, FlipViewController controller,
				int pageIndex)
		{
		...
		protected void onPreExecute()
		{
			...
			// Commented out; pd and imgBackground should not be accessed
			// directly
			/*
		...
		protected void onPostExecute(Bitmap result)
		{
			...
			{ // make sure the ImageView instance has not been reused for
				// another page
				if (result != null)
				{
			...
			FlipViewController controller, int pageIndex)
	{
		...
		if (shouldStartAsyncLoad(imgIcon, mGalleryPage.getTargetURL(),
				pageIndex))
		{
			ContentPhotosDownloader downloader = new ContentPhotosDownloader(
					mGalleryPage.getTargetURL(), imgIcon, null, controller,
					pageIndex);
			imgIcon.setImageDrawable(new AsyncDrawable(getResources(), null,
					downloader));
			downloader.execute();
		}
		...
		if (shouldStartAsyncLoad(imgBackground, mGalleryPage.getImageURL(),
				pageIndex))
		{
			ContentPhotosDownloader downloader = new ContentPhotosDownloader(
					mGalleryPage.getImageURL(), imgBackground, pd, controller,
					pageIndex);
			imgBackground.setImageDrawable(new AsyncDrawable(getResources(),
					null, downloader));
			downloader.execute();
		}
	}

	private boolean shouldStartAsyncLoad(ImageView imageView, String url,
			int pageIndex)
	{
		...
		if (downloader != null)
		{
			if (downloader.getPageIndex() == pageIndex
					&& url.equals(downloader.getUrl()))
			{
				shouldStart = false;
			}
			else
			...
			FlipViewController controller, int pageIndex)
	{
		...
		// notes: it's not the right approach to make the background fill its
		// parent, using a RelativeLayout could be much better
		imgBackground.setLayoutParams(new LayoutParams(measuredwidth,
				measuredheight));
