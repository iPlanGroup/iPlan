...
public class VocationTable implements TableCreateInterface, VacationInterface{
	...
	private static VocationTable mVacationTable = new VocationTable();
	
	private VocationTable()
	{
		
	}
	
	public static VocationTable getInstance()
	{
	...
	public static TermInfo getTermInfoById(int vocationId)
	{
		...
		Cursor cursor = db.query(VocationTable.TABLE_NAME, null, 
				VACATION_ID+"=?", 
	...
	public void onCreate(SQLiteDatabase db) {
		String sql = "CREATE TABLE "
				+ VocationTable.TABLE_NAME 
				+ " ( "
				+ VocationTable.VACATION_ID + " integer primary key autoincrement, "	
				+ VocationTable.START_TIME + " long not null, "
				+ VocationTable.END_TIME + " long not null, "
				+ VocationTable.NAME + " varchar(1024), "
				+ VocationTable.IS_TERM + " boolean"
				+ " );";
		db.execSQL(sql);
	...
	public void onUpgrade(SQLiteDatabase db, int oldVersion, int newVersion) {
		...
		if (oldVersion < newVersion) {
			String sql = "DROP TABLE IF EXISTS " + VocationTable.TABLE_NAME;
			db.execSQL(sql);
			this.onCreate(db);
		}
